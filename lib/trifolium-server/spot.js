var Action, MoveAction, SearchAction, SharedItemCreator, Spot, WaitAction, crypto, _ref;
...
Spot = (function() {
  ...
  function Spot(name, posX, posY, actionInfoList) {
    ...
    date = new Date;
    this.id = crypto.createHash('sha1').update("" + this.name).update("" + this.posX + "-" + this.posY).update('6b1d6cfdb9889216').update("" + (date.getTime())).update("" + (date.getMilliseconds())).digest('hex');
  ...
  Spot.prototype.details = function() {
    return {
      id: this.id,
      name: this.name,
      posX: this.posX,
